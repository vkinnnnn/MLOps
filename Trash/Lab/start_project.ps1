...
foreach ($service in $services) {
    ...
    if ($container -eq $service.Container) {
        ...
        Write-Host "  [X] $($service.Name) is not running" -ForegroundColor Red
    }
}
...
foreach ($service in $servicesStatus.GetEnumerator()) {
    $status = if ($service.Value) { "[OK] RUNNING" } else { "[...] STARTING" }
...
if ($allRunning) {
    Write-Host "[SUCCESS] Project started successfully!" -ForegroundColor Green
    Write-Host "  Open http://localhost:5173 in your browser to get started." -ForegroundColor White
} else {
    Write-Host "[WARNING] Some services are still starting..." -ForegroundColor Yellow
